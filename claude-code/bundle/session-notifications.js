...
function sessionBlock(next, sid, date) {
  ...
  const meta = [sid ? `session ${sid}` : "", age].filter(Boolean).join(" \xB7 ");
  return `   \u{1F4CC} ${next}
` + (meta ? `   \u21B3 ${meta}
` : "");
}
...
async function pickResumeBrief(creds, currentSessionId) {
  ...
  try {
    ...
    if (blocks.length > 0) {
      ...
      return {
        brief: `Picking up on ${project} \u2014 where you left off:
` + blocks.join("") + `   Ask me for the full thread whenever you're ready.`
      };
    }
