...
// Unified hivemind dispatcher (the npm `bin: hivemind` entry). Lives one level
// above each agent's bundle dir (e.g. <root>/claude-code/bundle/.. → <root>/bundle/cli.js).
...
async function main(): Promise<void> {
  ...
  const resolvedContext = context
    .replace(/HIVEMIND_AUTH_CMD/g, AUTH_CMD)
    .replace(/HIVEMIND_CLI/g, HIVEMIND_CLI);
