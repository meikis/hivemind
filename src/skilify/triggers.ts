 ...
 *
 * Both are no-ops when cwd is empty or HIVEMIND_SKILIFY_WORKER=1 (recursion
 * guard). Both spawn the worker as a detached subprocess via
 * spawnSkilifyWorker; they never block the calling hook.
 */

import type { Config } from "../config.js";
import { spawnSkilifyWorker, skilifyLog } from "./spawn-skilify-worker.js";
import {
  bumpStopCounter,
  resetCounter,
...
export function tryStopCounterTrigger(opts: TriggerOptions): void {
  if (process.env.HIVEMIND_SKILIFY_WORKER === "1") return;
  if (!opts.cwd) return;
